  ...
  try {
    ...
    for (const field of updateFields) {
      if (req.body[field] !== undefined) {
        await update('users', 'id', id, field, req.body[field]);
      }
    }
 ...
 * /api/users/generate:
 *   post:
 *     summary: Generate multiple users
 *     tags: [Users]
 *     security:
 *       - BearerAuth: []
 *     requestBody:
...
router.post('/generate', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const { count, prefix, password = 'password123' } = req.body;
    
    if (!count || !prefix) {
      return res.status(400).json({ 
        error: 'count and prefix are required' 
      });
    }
    ...
    res.status(201).json({
      message: `${createdUsers.length} users generated successfully`,
      users: createdUsers
    });
  } catch (error) {
