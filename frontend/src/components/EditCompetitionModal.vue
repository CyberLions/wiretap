  ...
  <Teleport to="body">
    ...
    <div class="bg-gray-800 rounded-lg shadow-xl border border-gray-700 w-full max-w-md mx-4">
      <div class="px-6 py-4 border-b border-gray-700">
        <h3 class="text-lg font-medium text-white">Edit Competition</h3>
      </div>
      ...
      <form @submit.prevent="handleSubmit" class="p-6 space-y-4">
        ...
        <div class="flex justify-end space-x-3 pt-4">
          ...
          >
            ...
            <span v-else>Update Competition</span>
          </button>
        </div>
      </form>
    </div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const populateForm = (competition) => {
      if (competition) {
        form.value = {
          name: competition.name || '',
          description: competition.description || '',
          provider_id: competition.provider_id || '',
          openstack_project_name: competition.openstack_project_name || ''
        }
      }
    }
    ...
    const handleSubmit = async () => {
      ...
      loading.value = true
      try {
        await api.workshops.update(props.competition.id, form.value)
        emit('updated')
        emit('close')
      } catch (error) {
        console.error('Error updating competition:', error)
        // Emit error event for parent to handle
        emit('error', error.response?.data?.error || 'Failed to update competition')
      } finally {
        loading.value = false
      }
    }
    ...
    // Populate form when competition changes
    watch(() => props.competition, (newCompetition) => {
      if (newCompetition) {
        populateForm(newCompetition)
        // Fetch projects if provider is already set
        if (newCompetition.provider_id) {
          fetchProjects(newCompetition.provider_id)
        }
      }
    }, { immediate: true })
