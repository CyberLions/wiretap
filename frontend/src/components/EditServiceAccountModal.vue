  ...
  <div v-if="show" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-gray-800 rounded-lg shadow-xl border border-gray-700 w-full max-w-md mx-4">
      <div class="px-6 py-4 border-b border-gray-700">
        <h3 class="text-lg font-medium text-white">Edit Service Account</h3>
      </div>
      ...
      <form @submit.prevent="handleSubmit" class="p-6 space-y-4">
        ...
        <div class="flex justify-end space-x-3 pt-4">
          ...
          >
            ...
            <span v-else>Update Service Account</span>
          </button>
        </div>
      </form>
    </div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const handleSubmit = async () => {
      loading.value = true
      try {
        await api.admin.updateServiceAccount(props.serviceAccount.id, form.value)
        emit('updated')
        emit('close')
        resetForm()
      } catch (error) {
        ...
        emit('error', error.response?.data?.error || 'Failed to update service account')
      } finally {
        loading.value = false
      }
    }

    // Reset form when modal closes or service account changes
    ...
    watch(() => props.serviceAccount, (newVal) => {
      if (newVal) {
